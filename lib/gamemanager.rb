...
class GameManager
  ...
  def moves_when_in_check(king)
    available_moves = []
    @active_pieces.each do |_k, piece|
      next unless piece.color == king.color

      # Check if this piece can move to a square that gets the king out of check
      piece.possible_moves(@board).each do |move|
        temp_board = @board.dup
        temp_board.set_piece(piece, move)
        available_moves << [piece, move] unless temp_board.square_under_attack?(king.position, opposite_color, king)
      end
    end
    available_moves
  end
